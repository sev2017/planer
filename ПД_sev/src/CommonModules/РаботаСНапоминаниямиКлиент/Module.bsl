...
Функция ПредставлениеВремени(Знач Время, ПолноеПредставление = Истина, ВыводитьСекунды = Истина) Экспорт
	...
	Иначе
		...
		Если КоличествоДней > 0 Тогда
			Результат = Результат + ОбщегоНазначения.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоДней, ПредставлениеДней) + " ";
			Счетчик = Счетчик + 1;
		КонецЕсли;
		
		Если КоличествоЧасов > 0 Тогда
			Результат = Результат + ОбщегоНазначения.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоЧасов, ПредставлениеЧасов) + " ";
			Счетчик = Счетчик + 1;
		КонецЕсли;
		
		Если (ПолноеПредставление Или Счетчик < 2) И КоличествоМинут > 0 Тогда
			Результат = Результат + ОбщегоНазначения.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоМинут, ПредставлениеМинут) + " ";
			Счетчик = Счетчик + 1;
		КонецЕсли;
		...
		Если (ПолноеПредставление Или Счетчик < 2) И (КоличествоСекунд > 0 Или КоличествоНедель+КоличествоДней+КоличествоЧасов+КоличествоМинут = 0) Тогда
			Результат = Результат + ОбщегоНазначения.ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоСекунд, ПредставлениеСекунд);
		КонецЕсли;
...
Функция ПолучитьТекущиеОповещения(ВремяБлижайшего = Неопределено) Экспорт
	...
	//ТаблицаОповещений = ОбщегоНазначенияКлиентПовтИсп.ПолучитьНапоминанияТекущегоПользователя();
	ТаблицаОповещений = РаботаСНапоминаниямиСервер.ПолучитьНапоминанияТекущегоПользователя();
	Результат = Новый Массив;
	
	ВремяБлижайшего = Неопределено;
	ТекДата = ТекущаяДата();	
	Для Каждого Оповещение Из ТаблицаОповещений Цикл
		Если Оповещение.СрокНапоминания <= ТекДата Тогда
...
Процедура ОткрытьФормуОповещения() Экспорт
	...
	ИмяПараметра = "ОбщаяФорма.ФормаОповещения";
	...
	ОткрытьФорму(ИмяПараметра,,,,Окно);
КонецПроцедуры
...
Процедура НапомнитьВУказанноеВремя(стрПараметры) Экспорт 
	РаботаСНапоминаниямиСервер.СоздатьНапоминание(стрПараметры);
КонецПроцедуры
